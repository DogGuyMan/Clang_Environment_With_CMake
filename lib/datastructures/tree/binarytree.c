...
static const BinaryTree__TYPE_NAME__ DEFAULT_BINARYTREE___TYPE_NAME__SPSP__TEMPLATE = {
	.m_data = 0,
	.m_size = 0,
	.m_max_depth = 0,
	.m_root = NULL,
	...
	size_t added_size)
{
	...
	if (current_tree != self_ptr->m_root)
	{
		BottomUpBinaryTreeUpdateSize(self_ptr, current_tree->m_parent, added_size);
	}
	return;
}

void BinaryTree__TYPE_NAME___SPSP_DataSwap(
	BinaryTree__TYPE_NAME__ *lhs, BinaryTree__TYPE_NAME__ *rhs)
{
	if (lhs == NULL || rhs == NULL)
		abort();
	__TYPE__ tval = lhs->get(lhs, lhs);
	lhs->m_data = rhs->m_data;
	rhs->m_data = tval;
}

void BinaryTree__TYPE_NAME___SPSP_LeftReconnectSubtree(
	BinaryTree__TYPE_NAME__ *parent, BinaryTree__TYPE_NAME__ *left_subtree)
{
	if (parent == NULL || left_subtree == NULL) abort();

	BinaryTree__TYPE_NAME__ *rm_subtree = parent->m_left_child;
	...
	else if (left_subtree->m_parent->m_right_child == left_subtree)
	{
		...
		abort();
	DestroyBinaryTree__TYPE_NAME__(rm_subtree);
	parent->insert(parent->m_root, parent, LEFT_INSERT_NODE, left_subtree);
}

void BinaryTree__TYPE_NAME___SPSP_RightReconnectSubtree(
	BinaryTree__TYPE_NAME__ *parent, BinaryTree__TYPE_NAME__ *right_subtree)
{
	if (parent == NULL || right_subtree == NULL) abort();

	BinaryTree__TYPE_NAME__ *rm_subtree = parent->m_right_child;
	rm_subtree->m_parent = NULL;
	rm_subtree->m_root = rm_subtree;
	rm_subtree->m_current = rm_subtree;
...
BinaryTree__TYPE_NAME__ *CreateBinaryTree__TYPE_NAME__(__TYPE__ data)
{
	...
	memcpy(temp_tree, &DEFAULT_BINARYTREE___TYPE_NAME__SPSP__TEMPLATE, sizeof(BinaryTree__TYPE_NAME__));
...
size_t BinaryTree__TYPE_NAME___SPSP_Size(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree)
{
...
size_t BinaryTree__TYPE_NAME___SPSP_MaxDepth(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree)
{
	return current_tree->m_max_depth;
}

BinaryTree__TYPE_NAME__ *BinaryTree__TYPE_NAME___SPSP_LeftChild(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree)
{
	return current_tree->m_left_child;
}

BinaryTree__TYPE_NAME__ *BinaryTree__TYPE_NAME___SPSP_RightChild(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree)
{
...
BinaryTree__TYPE_NAME__ *BinaryTree__TYPE_NAME___SPSP_Insert(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree,
	E_BINARY_TREE_INSERT_MODE insert_mode, BinaryTree__TYPE_NAME__ *insert_tree)
{
	...
	one start
	*/
	BottomUpBinaryTreeUpdateSize(self_ptr, new_tree->m_parent, new_tree->m_size);
	/*
	zero depth
	*/
...
__TYPE__ BinaryTree__TYPE_NAME___SPSP_Get(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree)
{
	if (self_ptr == NULL || current_tree == NULL)
	{
...
BinaryTree__TYPE_NAME__ *BinaryTree__TYPE_NAME___SPSP_Remove(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *sub_tree)
{
	...
	*/
	...
	BottomUpBinaryTreeUpdateSize(self_ptr, parent_tree->m_parent, -rm_tree->m_size);
	BottomUpBinaryTreeUpdateDepth(self_ptr, parent_tree->m_parent);

	return rm_tree;
}

void BinaryTree__TYPE_NAME___SPSP_BFS(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree, void *user_data)
{
...
void BinaryTree__TYPE_NAME___SPSP_LevelOrder(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree, void *user_data)
{
}

void BinaryTree__TYPE_NAME___SPSP_RecurseDFSPreorder(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree, void *user_data)
{
}

void BinaryTree__TYPE_NAME___SPSP_RecurseDFSInorder(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree, void *user_data)
{
}

void BinaryTree__TYPE_NAME___SPSP_RecurseDFSPostorder(
	BinaryTree__TYPE_NAME__ *self_ptr, BinaryTree__TYPE_NAME__ *current_tree, void *user_data)
{
}
